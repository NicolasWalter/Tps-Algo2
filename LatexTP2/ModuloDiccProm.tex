...
\disDondeEs{estr}{\disTuplaEstr{nat/CantClaves, arreglo $de \ lista (datos)$/tabla}}
...
\begin{algorithm}[H]
\begin{algorithmic}[1]
\Function {\textsc{iVacio}}{\paramIn{n}{nat} }{$\disFlecha$ res : estr} \Comment{$\Ode{cantClaves}$}
	\State var arreglo(lista(datos)) tabla $\gets$ crearArreglo[n] \Comment{$\Ode{cantClaves}$}
	\State \textbf{for} i $\gets$ 0 \textbf{to} n \textbf{do} \Comment{$\Ode{cantClaves}$}
